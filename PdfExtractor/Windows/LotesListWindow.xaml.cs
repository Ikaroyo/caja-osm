...
namespace PdfExtractor.Windows
{
    public partial class LotesListWindow : Window
    {
        ...
        private void BtnBorrarLotes_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                // Obtener lotes seleccionados
                var lotesSeleccionados = filteredLotes.Where(l => l.IsSelected).ToList();
                
                if (lotesSeleccionados.Count == 0)
                {
                    MessageBox.Show("No hay lotes seleccionados para borrar.", "Aviso", 
                        MessageBoxButton.OK, MessageBoxImage.Information);
                    return;
                }

                // Mostrar confirmación
                string mensaje = $"¿Está seguro que desea borrar {lotesSeleccionados.Count} lote(s) seleccionado(s)?\n\n";
                mensaje += "Esta acción no se puede deshacer.\n\n";
                mensaje += "Lotes a borrar:\n";
                
                foreach (var lote in lotesSeleccionados.Take(5)) // Mostrar máximo 5 para no hacer el mensaje muy largo
                {
                    mensaje += $"- {lote.Dia} {lote.FechaString} Lote:{lote.Lote} Usuario:{lote.Usuario} Caja:{lote.Caja}\n";
                }
                
                if (lotesSeleccionados.Count > 5)
                {
                    mensaje += $"... y {lotesSeleccionados.Count - 5} más\n";
                }

                var result = MessageBox.Show(mensaje, "Confirmar Borrado", 
                    MessageBoxButton.YesNo, MessageBoxImage.Warning);

                if (result == MessageBoxResult.Yes)
                {
                    // Borrar lotes de la lista principal
                    foreach (var lote in lotesSeleccionados)
                    {
                        allLotes.Remove(lote);
                    }

                    // Guardar cambios
                    DataService.SaveData(allLotes, saveLocation);
                    
                    // Recargar datos
                    LoadData();
                    
                    MessageBox.Show($"Se borraron {lotesSeleccionados.Count} lote(s) exitosamente.", "Éxito", 
                        MessageBoxButton.OK, MessageBoxImage.Information);
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error borrando lotes: {ex.Message}", "Error", 
                    MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
    }
